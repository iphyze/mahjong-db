...
try {
    ...
    if ($loggedInUserRole !== 'Admin' && $loggedInUserRole !== "Super_Admin") {
        throw new Exception("Unauthorized: Only Admins can access game_days", 401);
    }
    ...
    // Sorting
    $sortableFields = ['id', 'createdAt', 'name', 'title'];
    ...
    // Search condition
    if (!empty($search)) {
        $whereClause = "WHERE name LIKE ? OR title LIKE ?";
        $params[] = '%' . $search . '%';
        $params[] = '%' . $search . '%';
        $types .= 'ss';
    }
    ...
    // Total count query
    $countQuery = "SELECT COUNT(*) AS total FROM game_days $whereClause";
    ...
    $dataQuery = "SELECT * FROM game_days $whereClause ORDER BY $sortBy $sortOrder LIMIT ? OFFSET ?";
    ...
    $game_days = $result->fetch_all(MYSQLI_ASSOC);
    $dataStmt->close();
