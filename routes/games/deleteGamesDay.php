...
try {
    ...
    $checkGamesQuery = "SELECT id FROM game_days WHERE id IN ($placeholders)";
    ...
    $deleteInterestQuery = "DELETE FROM game_interests WHERE game_id IN ($placeholders)";
    $stmt = $conn->prepare($deleteInterestQuery);
    if (!$stmt) {
        throw new Exception("Database preparation error (interest): " . $conn->error, 500);
    }
    ...
    $deleteParticipantsQuery = "DELETE FROM game_participants WHERE game_id IN ($placeholders)";
    $stmt = $conn->prepare($deleteParticipantsQuery);
    if (!$stmt){
        throw new Exception("Database preparation error (participants): " . $conn->error, 500);
    } 
    ...
    $deletePairsQuery = "DELETE FROM game_pairs WHERE gameId IN ($placeholders)";
    ...
    // Delete the games
    $deleteGamesQuery = "DELETE FROM game_days WHERE id IN ($placeholders)";
